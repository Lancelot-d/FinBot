...
def update_chat(n_clicks, user_input, chat_history):
    ...
    response = chat.invoke(user_input + "\n ###Always format response in markdown format###").content
    chat_history.append(f"**Bot:** \n{response}\n")  # Placeholder for bot response
    return {'chat_history': chat_history}

@dash.callback(
    output={
        'chat_display': Output("chat-display", "children")
    },
    inputs={
        'chat_history': Input("chat-history", "data")
    },
    state={},
    prevent_initial_call=True
)
        ...
        'chat_display': html.Div(
            [
                html.Div(
                    ...
                    style={
                        "backgroundColor": "#f9f9f9" if "**You:**" in msg else "#333333",
                        "borderRadius": "10px",
                        "padding": "10px",
                        "margin": "5px 0",
                        "maxWidth": "90%", 
                ...
                for msg in chat_history if "**You:**" in msg or "**Bot:**" in msg
            ],
            style={
                "display": "flex",
